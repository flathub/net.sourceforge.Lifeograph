...
: NamedElement( STR0/SI::NEW_COLUMN ), m_id( id ), m_p2data( p2data ), m_type( t )
{ }
...
TableColumn::is_percentage() const
{
    switch( m_type )
    {
        ...
        case TCT_TAG_V:
            return( m_opt_int == VT::COMPL_PERCENTAGE );
        default:
            return false;
    }
}
...
TableColumn::allocate_filter_stacks()
{
    if( m_type == TCT_BOOL )
    {
        auto filter{ m_p2data->m_p2diary->get_filter( m_opt_int ) };
        if( filter )
...
TableColumn::calculate_value_num( const Entry* entry, TableLine* line ) const
{
    ...
    if( entry )
    {
        switch( m_type )
        {
            ...
            case TCT_TAG_V:
                if( m_p2tag )
                {
                    const auto  F_avg{ bool( m_opt_int & VT::AVERAGE ) };
                    Value       v;

                    if     ( m_opt_int & VT::REMAINING )
                        v = entry->get_tag_value_remaining( m_p2tag, F_avg );
                    else if( m_opt_int & VT::PLANNED )
                        v = entry->get_tag_value_planned( m_p2tag, F_avg );
                    else if( m_opt_int == VT::COMPL_PERCENTAGE )
                    {
    ...
    line->set_col_v_num( m_index, value, weight );
...
TableColumn::calculate_value_txt( const Entry* entry ) const
{
    ...
    switch( m_type )
    {
        ...
        case TCT_NUMBER:
            return std::to_string( entry->get_sibling_order() );
        case TCT_SUB:
...
TableColumn::calculate_value_tag( const Entry* entry ) const
{
    ...
    if( m_type == TCT_SUB && m_p2tag && !is_counting() && m_opt_int != VT::TCOS_ALL )
    {
        switch( m_opt_int )
        {
...
TableColumn::calculate_value_gantt( const Entry* entry, TableLine* line ) const
{
    ...
    line->set_col_v_num( m_index, Date::get_secs_since_min( line->m_periods.front().dbgn ) );
}
        ...
        switch( m_type )
        {
            ...
            case TCT_SUB:
                if( m_p2tag && is_counting() )
                {
                    ...
                    switch( m_opt_int )
                    {
                        case VT::FIRST:   tag_sub = para->get_sub_tag_first( m_p2tag ); break;
                        case VT::LAST:    tag_sub = para->get_sub_tag_last( m_p2tag ); break;
                        case VT::LOWEST:  tag_sub = para->get_sub_tag_lowest( m_p2tag ); break;
                        case VT::HIGHEST: tag_sub = para->get_sub_tag_highest( m_p2tag ); break;
                        case VT::TCOS_ALL:
                        {
...
TableColumn::calculate_value_txt( const Paragraph* para ) const
{
    ...
    {
        case TCT_TEXT:
            if( ( m_opt_int & VT::SEQ_FILTER ) == VT::TCT_SRC_URI )
                return para->get_uri_broad();
            else
                return para->get_text_stripped( m_opt_int );
        case TCT_NUMBER:
            return std::to_string( para->m_order_in_host );
        case TCT_THEME:
...
TableColumn::calculate_value_tag( const Paragraph* para ) const
{
    ...
    if( para && m_type == TCT_SUB && m_p2tag && !is_counting() )
    {
        switch( m_opt_int )
        {
            case VT::FIRST:   return para->get_sub_tag_first( m_p2tag ); break;
            case VT::LAST:    return para->get_sub_tag_last( m_p2tag ); break;
            case VT::LOWEST:  return para->get_sub_tag_lowest( m_p2tag ); break;
            case VT::HIGHEST: return para->get_sub_tag_highest( m_p2tag ); break;
            default:
                break;
        }
    }
...
TableColumn::calculate_value_date( const Paragraph* para ) const
{
    ...
    if( para && m_type == TCT_DATE )
    {
        switch( m_opt_int & VT::BODY_FILTER )
        {
            case VT::DATE_CREATION: return para->m_host->get_date_created(); break;
            case VT::DATE_CHANGE:   return para->m_host->get_date_edited(); break;
...
Value
TableColumn::calculate_duration( const DiaryElemDataSrc* elem, TableLine* line ) const
{
    ...
    switch( m_opt_int & VT::TCD_FILTER_BGN )
    {
        ...
        case VT::TCD_BGN_L1:
            d_bgn = ( m_index > 0 ? line->get_col_v_date( m_index - 1 ) : 0 ); break;
        case VT::TCD_BGN_L2:
            d_bgn = ( m_index > 1 ? line->get_col_v_date( m_index - 2 ) : 0 ); break;
    }
    ...
    switch( m_opt_int & VT::TCD_FILTER_END )
    {
        ...
        case VT::TCD_END_L1:
            d_end = ( m_index > 0 ? line->get_col_v_date( m_index - 1 ) : 0 ); break;
        case VT::TCD_END_L2:
            d_end = ( m_index > 1 ? line->get_col_v_date( m_index - 2 ) : 0 ); break;
    }
...
TableColumn::calculate_arithmetic( TableLine* line ) const
{
    ...
    if( m_opt_int1 > 0 )
    {
        ...
        vA = ( col ? line->get_col_v_num( col->m_index ) : 0.0 );
    ...
    if( m_opt_int2 > 0 )
    {
        ...
        vB = ( col ? line->get_col_v_num( col->m_index ) : 0.0 );
        // the unary modifier function
...
TableColumn::format_number( TableLine* line ) const
{
    ...
    const Value   num_value       { line->get_col_v_num( m_index ) };
    ...
    line->set_col_v_txt( m_index, formatted_str );
}

// TABLE LINE ======================================================================================
TableLine::TableLine( TableData* p2data, bool F_expanded )
: m_p2data( p2data ), m_sublines( p2data->m_columns_sort ), m_F_expanded( F_expanded )
{ }
...
TableLine::get_col_v_num_by_cid( int cid ) const
{
    ...
    return get_col_v_num( col->get_index() );
}
...
TableLine::get_col_v_txt_by_cid( int cid ) const
{
    ...
    return get_col_v_txt( col->get_index() );
}

// this is costlier than other get_value_* functions as dates are not cached for now
DateV
TableLine::get_col_v_date( unsigned int ic ) const
{
...
TableLine::get_col_v_date_by_cid( int cid ) const
{
    ...
    return get_col_v_date( col->get_index() );
}
...
void
TableLine::set_col_v_num( const int i, Value v, Value w )
{
...
void
TableLine::set_col_v_txt( int i, const Ustring& v )
{
...
TableLine::add_col_v_gantt( int ic, TableLine* sl )
{
    ...
    set_col_v_num( ic, m_periods.empty() ? 0 : Date::get_secs_since_min( m_periods.front().dbgn ) );
}
...
TableLine*
TableLine::get_subline_by_id( DEID id ) const
{
    for( auto& sl : m_sublines )
    {
        if( sl->is_group_head() )
...
{
    TableLine*  gl { new TableLine( m_p2data, sl->is_expanded() ) };
    const auto  ic { ( * m_p2data->m_columns_sort.begin() )->get_index() };

    gl->incorporate_subline_values( sl );
    gl->set_col_v_txt( ic, sl->get_col_v_txt( ic ) );
    gl->m_p2parent = this;

    m_subgroups_unsorted.push_back( gl );
    gl->m_sublines.insert( sl );

    gl->m_depth = ( m_depth + 1 );
    sl->m_depth = ( m_depth + 2 );
...
TableLine::incorporate_subline_values( TableLine* sl )
{
    for( auto& col : m_p2data->m_columns )
    {
        ...
        // only for sorting:
        else if( col->get_type() == TableColumn::TCT_SUB && col->get_opt_int() != VT::TCOS_ALL )
            set_col_v_num( ic, col_v );
        ...
        if( col_v < col->m_value_min ) col->m_value_min = col_v;
        if( col_v > col->m_value_max ) col->m_value_max = col_v;
    }
}
    ...
    for( auto& col : m_p2data->m_columns )
    {
        ...
        switch( col->m_summary_func )
        {
            case VT::SUMF::SUM::C:
                ...
                break;
            case VT::SUMF::AVG::C:
                set_col_v_txt( ic, STR::format_number( get_col_v_num( ic ) /
                                                       get_col_weight( ic ) ) );
                break;
            default:
                break;
...
TableLine::insert_subline( TableLine* line )
{
    ...
    if( m_depth >= m_p2data->m_grouping_depth ) // add as a child
    {
    ...
    for( auto it = m_sublines.begin(); it != m_sublines.end(); )
    {
        ...
        if( sl->m_p2elem )
        {
                ...
                sl->m_p2elem->get_id() == entry->get_id() )
            {
                ...
                delete sl;
                it = m_sublines.erase( it ); // causes occasional crashes if comes before delete. why?
    ...
    for( auto& col : m_columns )
    {
        ...
        switch( ct )
        {
            ...
            case TableColumn::TCT_TAG_V:
                switch( vt & VT::BODY_FILTER )
                {
                    case VT::TOTAL_PLANNED:     table_def += "TP~~~~~"; break;
                    case VT::TOTAL_REMAINING:   table_def += "TB~~~~~"; break; // B for balance
                    case VT::AVG_REALIZED:      table_def += "Tr~~~~~"; break;
                    case VT::AVG_PLANNED:       table_def += "Tp~~~~~"; break;
                    case VT::AVG_REMAINING:     table_def += "Tb~~~~~"; break;
                    case VT::COMPL_PERCENTAGE:  table_def += "TC~~~~~"; break;
                    default:                    table_def += "TR~~~~~"; break; // TOTAL_REALIZED
                }
                break;
        ...
        if( ( col->get_source_elem() == VT::SRC_PARENT_FILTER ||
            ...
            table_def += STR::compose( "Mcgf", col->get_source_elem_filter()->get_id(), '\n' );
        ...
        if( col->get_count_filter() != nullptr )
            table_def += STR::compose( "Mccf", col->get_count_filter()->get_id(), '\n' );
    }

    if( m_filter_entry != nullptr )
        table_def += STR::compose( "Mf", m_filter_entry->get_id(), '\n' );

    if( m_filter_para != nullptr )
        table_def += STR::compose( "Ml", m_filter_para->get_id(), '\n' );
...
TableData::set_from_string( const Ustring& def )
{
    ...
    while( STR::get_line( def, line_offset, line ) )
    {
        ...
        switch( line[ 1 ] )
        {
                ...
                switch( line[ 2 ] )
                {
                    ...
                    case 'o':   // options
                        switch( line[ 3 ] )
                        {
                            ...
                            //case 'C': break; used to be count. dropped
                            ...
                            case 'R': col->set_type( TableColumn::TCT_DURATION );
                                ...
                                break;
                            case 'S': col->set_type( TableColumn::TCT_SUB );
                                switch( line[ 4 ] )
                                {
                            ...
                            case 'T': col->set_type( TableColumn::TCT_TAG_V );
                                switch( line[ 4 ] )
                                {
                                    case 'P': col->set_opt_int( VT::TOTAL_PLANNED ); break;
                                    case 'B': col->set_opt_int( VT::TOTAL_REMAINING ); break;
                                    case 'r': col->set_opt_int( VT::AVG_REALIZED); break;
                                    case 'p': col->set_opt_int( VT::AVG_PLANNED ); break;
                                    case 'b': col->set_opt_int( VT::AVG_REMAINING ); break;
                                    case 'C': col->set_opt_int( VT::COMPL_PERCENTAGE ); break;
                                    default:  col->set_opt_int( VT::TOTAL_REALIZED); break;
                                }
                                break;
                    ...
                    case 'g':   // get from source
                        col->set_source_elem_filter(
                                m_p2diary->get_filter( std::stoul( line.substr( 4 ) ) ) );
                        break;
                    case 't':   // column tag
                        col->set_tag(
                                m_p2diary->get_entry_by_id( std::stoul( line.substr( 3 ) ) ) );
                        break;
                    case 'c':   // conditions
                        switch( line[ 3 ] )
                        {
            ...
            case 'f':   // filter
                m_filter_entry = m_p2diary->get_filter( std::stoul( line.substr( 2 ) ) );
                break;
            case 'l':   // filter
...
TableData::move_column( int i_cur, int i_tgt )
{
    ...
    {
        const auto v_txt { line->get_col_v_txt( i_cur ) };
        const auto v_num { line->get_col_v_num( i_cur ) };
        const auto weight{ line->get_col_weight( i_cur ) };

        line->set_col_v_txt( i_cur, line->get_col_v_txt( i_tgt ) );
        line->set_col_v_num( i_cur, line->get_col_v_num( i_tgt ), line->get_col_weight( i_tgt ) );
        line->set_col_v_txt( i_tgt, v_txt );
        line->set_col_v_num( i_tgt, v_num, weight );
    };
...
TableData::populate_lines( bool F_expanded, Entry* entry_single ) // use all entries if nullptr
{
    ...
    for( auto& col : m_columns )
    {
        ...
        if( !entry_single )
        {
            col->m_value_min = Constants::INFINITY_PLS;
            col->m_value_max = Constants::INFINITY_MNS;
        }
    }
         ...
         entry = ( entry_single ? nullptr : entry->get_next_straight() ) )
    {
        ...
        if( m_F_para_based )
        {
            for( Paragraph* para = entry->get_paragraph_1st(); para; para = para->m_p2next )
            {
                ...
                m_master_header.insert_subline( create_line( para, F_expanded ) );
                m_count_lines++;
                n_added++;
            }
        }
        ...
        {
            m_master_header.insert_subline( create_line( entry, F_expanded ) );
            m_count_lines++;
            n_added++;
        }
...
TableData::set_gantt_column_coords()
{
    ...
    m_date_bgn = date_bgn;
    m_master_header.set_col_v_num( m_p2col_gantt->m_index, 0.0, span );
}
...
TableData::update_delta_columns()
{
    ...
    for( ListTableColumns::size_type ic = 0; ic < col_count; ic++ )
    {
        ...
        for( int il = m_lines.size() - 1 ; il > 0; --il )
        {
            ...
            m_lines[ il ]->set_col_v_num( ic, v_num );
...
TableData::update_order_column()
{
    ...
    for( auto sl : m_lines )
    {
        ...
        sl->set_col_v_txt( 0, number );

        last_depth = sl->m_depth;
    }

    m_master_header.set_col_v_txt( 0, std::to_string( m_master_header.m_sublines.size() ) );
}
...
TableData::get_value( unsigned int il, unsigned int ic ) const
{
    if( il >= m_lines.size() || ic >= m_columns.size() )
        throw( LIFEO::Error( "table index out of bounds" ) );

    auto&& line { m_lines[ il ] };
    auto&& col  { m_columns[ ic ] };

    if( col->is_enumerable( true ) )
        return( line->get_col_v_num( ic ) );
    else
        return( 0.0 );
}
...
TableData::get_value_str( unsigned int il, unsigned int ic ) const
{
    ...
    return m_lines[ il ]->get_col_v_txt( ic );
}
...
TableData::get_total_value_str( unsigned int ic ) const
{
    ...
    return m_master_header.get_col_v_txt( ic );
}
...
const Ustring TableElem::DEFINITION_DEFAULT{ STR::compose(
        "\nMci", TableData::COL_ID_MIN, STR0/SI::NEW_COLUMN,
        "\nMcoX__~~~~~_S"
        "\nMcx____P_~~~~~~~~T"
        "\nMcw1.0" ) };
